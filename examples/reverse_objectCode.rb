...
def $main_symbol.H(expr)
  # reverse(cons(cons(true,nil_list),cons(cons(true,nil_list),cons(false,nil_list))))
  rhs = Box.new(Application.new($reverse_symbol,[Box.new(Application.new($cons_symbol,[Box.new(Application.new($cons_symbol,[Box.new(Application.new($true_symbol,[])),Box.new(Application.new($nil_list_symbol,[]))])),Box.new(Application.new($cons_symbol,[Box.new(Application.new($cons_symbol,[Box.new(Application.new($true_symbol,[])),Box.new(Application.new($nil_list_symbol,[]))])),Box.new(Application.new($cons_symbol,[Box.new(Application.new($false_symbol,[])),Box.new(Application.new($nil_list_symbol,[]))]))]))]))]))
  expr.replace(rhs.content)
  expr.H()
  expr
end
